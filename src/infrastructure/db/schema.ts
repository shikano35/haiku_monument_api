...
export const monuments = sqliteTable("monuments", {
...
export const inscriptions = sqliteTable("inscriptions", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  monumentId: integer("monument_id").notNull().references(() => monuments.id, {
    onDelete: "cascade",
  }),
...
export const poemAttributions = sqliteTable("poem_attributions", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  poemId: integer("poem_id").notNull().references(() => poems.id, {
    onDelete: "cascade",
  }),
  poetId: integer("poet_id").notNull().references(() => poets.id, {
    onDelete: "restrict",
  }),
...
export const monumentLocations = sqliteTable("monument_locations", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  monumentId: integer("monument_id").notNull().references(() => monuments.id, {
    onDelete: "cascade",
  }),
  ...
  assignedAt: text("assigned_at").default(sql`(DATETIME('now','localtime'))`).notNull(),
  ...
  index("monument_locations_unique_idx").on(table.monumentId, table.locationId),
]);

// Inscription Poems（碑文と句の関連）
export const inscriptionPoems = sqliteTable("inscription_poems", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  inscriptionId: integer("inscription_id").notNull().references(() => inscriptions.id, {
    onDelete: "cascade",
  }),
  poemId: integer("poem_id").notNull().references(() => poems.id, {
    onDelete: "cascade",
